...
| T-008 | Activate CI: get `.github/workflows/ci.yml` onto GitHub, then delete the pending file | OPEN | **Owner action required** (agent-side blocked, re-verified 2026-08-31): the App token is refused permission to create workflow files (two documented rejections). Options: (1) web UI — create `.github/workflows/ci.yml` with the content of `.github/pending/ci.yml` (also pasted in PR #1), or (2) grant the app the `workflows` repository permission. After the file lands: push trigger fires the first run; then delete the pending file and mark DONE |
