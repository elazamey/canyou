...
| T-011 | Thin-slice implementation: Tool Registry + Policy Gate + one Connector | OPEN | AUTHORIZED by OWNER DIRECTIVE §4-D (after ADR — now satisfied): minimal slice = Registry + Gate + Connector Interface + GitHub Connector + tests/permission boundaries + per-execution evidence; §5 exclusions binding |
