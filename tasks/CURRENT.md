...
| CI workflow installation | NOT INSTALLED | Re-verified 2026-08-31: API `contents/.github/workflows` → 404; `actions/workflows` → total_count 0; second push of the file rejected verbatim (`workflows` permission). Agent-side blocked; owner options in T-008 |
| CI execution | NOT VERIFIED | `actions/runs` → total_count 0 (2026-08-31); «No workflow runs yet» = evidence of absence, not pass/fail |
...
- **T-008 minimal-fix attempt executed and diagnosed (2026-08-31):** local `.github/workflows/` absent; staged trigger block valid (`on: push: branches: ["**"]` + `pull_request`); remote: `contents/.github/workflows` → 404, `actions/workflows` → total_count **0**, `actions/runs` → total_count **0**. Attempted the minimal fix — one commit moving `.github/pending/ci.yml` → `.github/workflows/ci.yml` (`a11ef66`) — and pushed; GitHub rejected it verbatim: `refusing to allow a GitHub App to create or update workflow .github/workflows/ci.yml without workflows permission` (second documented occurrence). The local commit was discarded (never pushed); staged state restored and verified clean. Conclusion: the sole blocker is platform permission — first run requires an owner action (T-008 options). «No workflow runs yet» is recorded as evidence of absence, not success or failure.
...
2. Owner: T-008 — activate CI (agent-side blocked; see two options in `tasks/BACKLOG.md`), record the first green run link here, then merge PR #1.
